...
class bot_motionplanner():
    ...
    def check_gtg_status(self, angle_to_turn, distance_to_goal):
        ...
        if ((abs(angle_to_turn) > 5) and (change_angle_to_turn < 0.4) and (not self.trigger_backpeddling)):
            ...
            self.angle_not_changed = 0
        print("[prev,change,not_changed_iter,self.trigger_backpeddling] = [{:.1f},{:.1f},{},{}] ".format(
            self.prev_angle_to_turn, change_angle_to_turn, self.angle_not_changed, self.trigger_backpeddling))
        ...
        if ((abs(distance_to_goal) > 5) and (change_dist < 1.2) and (not self.trigger_backpeddling)):
            ...
            self.dist_not_changed = 0
        print("[prev_d,change_d,not_changed_iter,self.trigger_backpeddling] = [{:.1f},{:.1f},{},{}] ".format(
            self.Prev_distance_to_goal, change_dist, self.dist_not_changed, self.trigger_backpeddling))
        ...
        # If mini-goal not changing for a long time
            ...
            self.goal_not_changed_long = 0
            self.goal_not_changed = 0
        print("[prev_g,change_g,not_changed_iter] = [{:.1f},{:.1f},{}] ".format(
            self.prev_path_iter, change_goal, self.goal_not_changed))
    ...
    def go_to_goal(self, bot_loc, path, velocity, velocity_publisher):
        ...
        angle_to_goal, distance_to_goal = self.angle_n_dist(
        ...
        print("angle to goal = {} Angle_to_turn = {} angle[Sim] {}".format(
            angle_to_goal, angle_to_turn, abs(angle)))
        ...
        if ((distance_to_goal <= 8) or self.trigger_nxtpt):
            ...
            if self.path_iter == (len(path)-1):
                ...
                if self.goal_not_reached_flag:
                    ...
                    # pygame.mixer.music.load(os.path.abspath(
                    #     'src/maze_bot/resource/Goal_reached.wav'))
