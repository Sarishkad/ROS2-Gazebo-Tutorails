...
class bot_mapper():
    ...
    def display_connected_nodes(self, curr_node, neighbor_node, case="Unkown", color=(0, 0, 255)):
        ...
        self.maze_connect = cv2.line(
            self.maze_connect, curr_pixel, neighbor_pixel, color, 1)
                        ...
                        if (paths == 3):
                            ...
                            if draw_intrstpts:
                                maze_bgr = self.triangle(
                                    maze_bgr, (col, row), 10, (144, 140, 255))
                            cv2.imshow("Maze (Interest Points)", maze_bgr)
                            # Adding [Found vertex to graph]
                            ...
                            if draw_intrstpts:
                                cv2.rectangle(
                                    maze_bgr, (col-10, row-10), (col+10, row+10), (255, 140, 144), 2)
                            cv2.imshow("Maze (Interest Points)", maze_bgr)
                            # Adding [Found vertex to graph]
    ...
    def graphify(self, extracted_maze):
        ...
        if not self.graphified:
            ...
            cv2.imshow('Maze (thinned*2)(Cropped)(Path_Overlayed)',
